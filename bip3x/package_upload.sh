...
TYPE=""
DRY_RUN=""
BUILD_ROOT="/Users/trietnguyen/Documents/Work/neil_projects/near-api-cpp/bip3x"
...
if [ ! -f "${BUILD_ROOT}/bip3x--Darwin-x86_64.sh" ]; then
  echo "File ${BUILD_ROOT}/bip3x--Darwin-x86_64.sh does not exists"
  exit 1
fi
...
if [ "${TYPE}" == "github" ]; then
  ...
  if [ "${DRY_RUN}" == "1" ]; then
    echo "${GHR_BIN} -replace 2.2.1 ${BUILD_ROOT}/bip3x--Darwin-x86_64.sh"
    exit 0
  fi

  ${GHR_BIN} -replace 2.2.1 ${BUILD_ROOT}/bip3x--Darwin-x86_64.sh
  ...
  # upload
  if [ "${DRY_RUN}" == "1" ]; then
    echo JFROG_CLI_OFFER_CONFIG=false ${JFROG_BIN} rt upload  "${BUILD_ROOT}/bip3x--Darwin-x86_64.sh" ""
    exit 0
  fi

  JFROG_CLI_OFFER_CONFIG=false ${JFROG_BIN} rt upload  "${BUILD_ROOT}/bip3x--Darwin-x86_64.sh" ""
